...
export default function ProductId({ params }: IProductIdProps) {
 ...
 const priceValue = (value: string) => {
  ...
  if (item) {
   ...
   setSelectPrice({
    oldPrice: data?.pricing_policy?.fixed?.price,
    offer: data?.pricing_policy?.fixed?.offer,
    price:
     data?.pricing_policy?.fixed?.price -
     data?.pricing_policy?.fixed?.price *
      (data?.pricing_policy?.fixed?.offer_value / 100),
    id: data?.pricing_policy?.fixed?._id
   });
  }
 };
 React.useEffect(() => {
  if (data?.pricing_policy?.price_type === 'fixed') {
   setSelectPrice({
      oldPrice: data?.pricing_policy?.fixed?.price,
      offer: data?.pricing_policy?.fixed?.offer,
      price:
       ...
       {data?.pricing_policy?.price_type === "flexible" ? (
        <ListItem sx={{ mt: 2 }}>
         <FormControl variant="outlined">
          ...
          >
           {data?.pricing_policy?.flexible?.map((value: any) => (
            <Box
             key={value._id}
             display={"flex"}
             alignItems={"center"}
