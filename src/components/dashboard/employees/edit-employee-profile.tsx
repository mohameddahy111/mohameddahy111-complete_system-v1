...
export interface IEditeEmployeeProfileProps {
    data :any
}

export default function EditeEmployeeProfile({data}: IEditeEmployeeProfileProps) {
  ...
  React.useEffect(()=>{
    Object.keys(formik.values.basic_Info).forEach(key => { 
     formik.setFieldValue(`basic_Info.${key}`, data[key])
    } )
  },[data])
  return (
    <form onSubmit={formik.handleSubmit}>
      ...
      >
        <Typography textTransform={'capitalize'} variant="h1" component="h1">
 edit {data.name}'s profile 
        </Typography>
